-- Add Strategic Indexes for Performance (Fixed Version)
-- Run each CREATE INDEX statement separately, or remove CONCURRENTLY

-- Option 1: Remove CONCURRENTLY (faster, but brief table locks)
...
CREATE INDEX IF NOT EXISTS idx_pods_event_id 
ON pods(event_id);

CREATE INDEX IF NOT EXISTS idx_pods_created_by 
ON pods(created_by);

-- POD_MEMBERS table indexes (chat membership checks)
CREATE INDEX IF NOT EXISTS idx_pod_members_pod_id 
ON pod_members(pod_id);

CREATE INDEX IF NOT EXISTS idx_pod_members_user_id 
ON pod_members(user_id);

-- MESSAGES table indexes (chat queries)
CREATE INDEX IF NOT EXISTS idx_messages_pod_id_created 
...
CREATE INDEX IF NOT EXISTS idx_events_date_utc 
ON events(date_utc);
...
CREATE INDEX IF NOT EXISTS idx_media_event_id_created 
...
CREATE INDEX IF NOT EXISTS idx_posts_event_id_created 
...
CREATE INDEX IF NOT EXISTS idx_memorabilia_event_user 
ON memorabilia(event_id, user_id);

-- MEET_POINTS table indexes
CREATE INDEX IF NOT EXISTS idx_meet_points_event_id 
